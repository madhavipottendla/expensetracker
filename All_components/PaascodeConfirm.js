...
const PasscodeConfirm = ({ route, navigation }) => {
  const [passcode, setPasscode] = React.useState('');
  const [error, setError] = React.useState('');
  const storedPasscode = route.params.storedPasscode;
...
const storePasscode = () => {
    if (passcode.length === 4) {
        if (passcode === storedPasscode) {
          console.log("Passcodes match. Proceeding to next page.");
          setError("");
  ...
  return (
    ...
    <View style={styles.container2}>
      <Text style={styles.title}>Ok. Re type your PIN again.</Text>
